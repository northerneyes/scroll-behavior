...
function useStandardScroll(createHistory) {
  ...
  function updateState(_ref) {
    ...
    var state = _historyLibDOMStateStorage.readState(currentKey);
    _historyLibDOMStateStorage.saveState(key, state);
    currentKey = key;
  }

  // `history` will invoke this listener synchronously, so `currentKey` will
  // always be defined.
  function updateScroll(_ref2) {
    var key = _ref2.key;
  ...
  return _utilsCreateUseScroll2['default'](updateState, updateScroll, start, stop)(createHistory);
}
